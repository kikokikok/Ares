...
impl NovaServer {
    ...
    pub async fn start(&self) -> NovaResult<()> {
        ...
        let listener = TcpListener::bind(&addr).await.map_err(|e| {
            NovaError::Generic(anyhow::anyhow!("Failed to bind to {}: {}", addr, e))
        })?;
        ...
        loop {
            match listener.accept().await {
                Ok((stream, addr)) => {
                    ...
                    let session_id =
                        format!("{}-{}", client_id, Instant::now().elapsed().as_millis());
    ...
    pub async fn send_proto_to_client<T: Message>(
        &self,
        client_id: Uuid,
        message: &T,
    ) -> NovaResult<()> {
    ...
    pub async fn send_to_client(&self, client_id: Uuid, data: &[u8]) -> NovaResult<()> {
        if let Some(client) = self.clients.get(&client_id) {
            match client.stream.lock().await.write_all(data).await {
                ...
                Err(e) => Err(NovaError::Generic(anyhow::anyhow!(
    ...
    pub async fn shutdown(&self) -> NovaResult<()> {
        ...
        info!(
            "Nova Server shutdown complete (uptime: {:.1}s)",
            uptime.as_secs_f64()
        );
