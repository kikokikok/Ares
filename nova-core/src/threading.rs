...
impl ThreadEngine {
    ...
    pub fn new(worker_threads: usize, main_thread_affinity: Option<usize>) -> NovaResult<Self> {
        ...
        let thread_pool = Arc::new(ThreadPoolBuilder::new()
            ...
            .build()
            .map_err(|e| NovaError::threading(format!("Failed to create thread pool: {}", e)))?);
        ...
        thread::Builder::new()
            ...
            .map_err(|e| NovaError::threading(format!("Failed to start scheduler thread: {}", e)))?;
        ...
        log::info!("Threading engine started with {} worker threads", worker_threads);
    ...
    pub fn submit_function<F>(&self, name: String, priority: TaskPriority, function: F) -> NovaResult<Uuid>
    where
    ...
    where
        T: Task + 'static,
    {
        let task_ids: Vec<Uuid> = tasks.into_iter()
                    ...
                    match task_wrapper.task.execute() {
                        Ok(()) => {
                            ...
                            stats.avg_execution_time_us = 
                                (stats.avg_execution_time_us + execution_time.as_micros() as f64) / 2.0;
...
impl Task for ComputeTask {
    fn execute(&self) -> NovaResult<()> {
        ...
        log::trace!("Compute task '{}' processed {} items in {:?} (sum: {})", 
                   self.name, self.work_amount, duration, sum);
...
impl Task for IOTask {
    fn execute(&self) -> NovaResult<()> {
        ...
        match std::fs::metadata(&self.file_path) {
            Ok(metadata) => {
                log::trace!("IO task '{}' read file {:?} (size: {} bytes)", 
                           self.name, self.file_path, metadata.len());
            }
            Err(e) => {
                log::warn!("IO task '{}' failed to read {:?}: {}", 
                          self.name, self.file_path, e);
            }
        }
