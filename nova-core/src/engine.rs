...
use crate::events::{EventSystem, EngineStartEvent, EngineStopEvent};
...
impl NovaEngine {
    ...
    pub async fn with_config(config: EngineConfig) -> NovaResult<Self> {
        ...
        let resource_manager = Arc::new(
            ResourceManager::new(&config.memory.heap_size, config.memory.gc_threshold)?
        );
        
        let event_system = Arc::new(
            EventSystem::new(config.events.queue_size, config.events.priority_levels)
        );
        
        let thread_engine = Arc::new(
            ThreadEngine::new(config.threading.worker_threads, config.threading.main_thread_affinity)?
        );
