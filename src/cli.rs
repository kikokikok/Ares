...
impl CliHandler {
    ...
    pub async fn start_server(&self, config: PathBuf, port: u16) -> Result<()> {
        info!(
            "Starting Nova Server on port {} with config: {:?}",
            port, config
        );
    ...
    pub async fn start_client(&self, config: PathBuf, server: String) -> Result<()> {
        info!(
            "Starting Nova Client connecting to {} with config: {:?}",
            server, config
        );
    ...
    pub async fn create_project(&self, name: String, path: Option<PathBuf>) -> Result<()> {
        ...
        let cargo_content =
            include_str!("../templates/project_cargo.toml").replace("{{project_name}}", &name);
    ...
    pub async fn create_plugin(&self, name: String) -> Result<()> {
        ...
        let lib_content =
            include_str!("../templates/plugin_lib.rs").replace("{{plugin_name}}", &name);
        std::fs::write(&lib_rs, lib_content)?;

        // Create Cargo.toml
        let cargo_toml = plugin_dir.join("Cargo.toml");
        let cargo_content =
            include_str!("../templates/plugin_cargo.toml").replace("{{plugin_name}}", &name);
        std::fs::write(&cargo_toml, cargo_content)?;

        // Create plugin.toml
        let plugin_toml = plugin_dir.join("plugin.toml");
        let plugin_content =
            include_str!("../templates/plugin.toml").replace("{{plugin_name}}", &name);
